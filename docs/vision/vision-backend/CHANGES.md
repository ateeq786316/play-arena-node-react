...
| 2026-07-28 | Auth module completion (OTP, refresh, profile, forgot/reset password) | prisma/schema.prisma, src/config/nodemailer.js, src/utils/emailTemplates.js, src/middlewares/auth.middleware.js, src/modules/auth/auth.route.js, src/modules/auth/auth.controller.js, src/modules/auth/auth.service.js, src/repository/auth.repo.js, server.js, src/constant/app.constant.js, docs/vision/postman-collection.json | Add OTP verification, JWT refresh, user profile, password reset flow |
...
- Created `docs/vision/vision-backend/` folder with 6 tracking `.md` files
...
- Created `docs/vision/postman-collection.json` with all auth API tests
...
- Created `docs/vision/vision-backend/requirement.md` — comprehensive single-source-of-truth for all 14 backend modules
...
### 2026-08-03 — Frontend ↔ Backend Integration Fixes
- **Fix 1: Upload route ordering** — `/:type` was registered before `/avatar`, `/tournament-poster`, `/ground-image/:groundId`, `/booking-proof/:groundId`, causing all specific uploads to be shadowed by the generic handler. Reordered routes so specific handlers come first.
  - Files: `src/modules/upload/upload.route.js`
- **Fix 2: Finance admin endpoint 401** — `getAdminFinance` in `finance.service.js` used hardcoded `userId === "admin-placeholder"` check instead of proper role-based authorization. Replaced with `requireAdmin` middleware on the route and removed the broken check from the service.
  - Files: `src/modules/finance/finance.route.js`, `src/modules/finance/finance.service.js`
- **Fix 3: Dispute role gates** — `/api/disputes/all` and `/api/disputes/:id/resolve` had no admin role gate. Added `requireAdmin` middleware to both routes.
  - Files: `src/modules/dispute/dispute.route.js`
- **Fix 4: React anti-pattern in disputes/new/page.tsx** — Used `useState(() => { api.get(...) })` which runs during render. Replaced with `useEffect` + proper loading state.
  - Files: `playarena-frontend/packages/web/src/app/(dashboard)/disputes/new/page.tsx`
- **Tests**: Added tests for finance admin endpoint and dispute admin role gates.
  - Files: `tests/finance.test.js`, `tests/dispute.test.js`